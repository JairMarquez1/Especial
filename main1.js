res = ['_','_','_','_','_','_','_','_','_','_','_','_','_','_']
let quantity = localStorage.getItem('quantityE');
let name = localStorage.getItem("aliasE");
...
function start(){
    ...
    let results = localStorage.getItem("resultsE");
    if (results){
    ...
    document.getElementById("total").innerHTML = "Total: $" + quantity*35 +"\n";
...
function number(){
    quantity = localStorage.getItem("quantityE");
    if (quantity)
        localStorage.setItem('quantityE', ++quantity);
    else{
        localStorage.setItem('quantityE', 1);
        quantity = localStorage.getItem("quantityE");}
    document.querySelector('.botonenviar span').textContent = quantity;
    localStorage.setItem('aliasE', name);
}

function result(){
    results = localStorage.getItem("resultsE");
    name = document.getElementById("nombre").value;
    name  = name.split('*').join('');
    if (results){
        if (aux > 1)
        localStorage.setItem('resultsE', results + "\n" + res.join("\xa0\xa0") + "\xa0\xa0" + name + " (" + aux + ")" + "*");
        else
        ...
        {
            if (aux > 1)
            localStorage.setItem('resultsE',res.join("\xa0\xa0") + "\xa0\xa0" + name + " (" + aux + ")" +  "*");
            else    
                localStorage.setItem('resultsE',res.join("\xa0\xa0") + "\xa0\xa0" + name+ "*");}       
}
...
function send(){
    ...
    whatsapptext = encodeURI(localStorage.getItem("resultsE"));
...
function recovername(){
    name = localStorage.getItem("aliasE");
...
function calculate(){
    ...
    quantity = localStorage.getItem("quantityE");
    if (quantity){
        localStorage.setItem('quantityE', parseInt(quantity)+aux);
        quantity = localStorage.getItem('quantityE');}
    else{
        localStorage.setItem('quantityE', aux);
        quantity = localStorage.getItem("quantityE");}
    document.querySelector('.botonenviar span').textContent = quantity;
    localStorage.setItem('aliasE', name);
}
...
function costoactual(){
    if (!res.join("\xa0\xa0").includes("_")){
        ...
        document.getElementById("costo").innerHTML = "Costo: $" + aux2*35;
        document.getElementById("numquinielas").innerHTML = aux2 + " Quiniela(s)"
    }
}
function remove(){
    if (quantity > 0){
        x = localStorage.getItem("resultsE");
        x = removeLastLine(x);
        localStorage.setItem("resultsE", x);
        results = x.split("*");
